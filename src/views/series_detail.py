import flet as ft
from core.state import state, Series, Episode
from core.theme import AppColors


def build_series_detail_view(
    page_obj: ft.Page,
    series: Series,
    on_load_episodes,
    on_play_episode,
    ...
    def build_episode_card(episode: Episode, idx: int):
        ...
        card = ft.Container(
            ...
            ink=True,
            on_click=lambda _: on_play_episode(series, idx),
    ...
    def update_episodes():
        ...
        if state.is_loading and not state.episodes:
            ...
            scroll_content.controls = [header, series_info, episodes_header, episodes_list]
    ...
    header = ft.Container(
        ...
        content=ft.Row(
            controls=[
                ft.Row(
                    controls=[back_btn, ft.Text(series.title, size=20, weight=ft.FontWeight.BOLD)],
                    spacing=12,
                ),
            ],
        )
    ...
    meta_row = ft.Row(
        controls=[
            ft.Text(series.year, color=AppColors.PRIMARY, weight=ft.FontWeight.BOLD, size=14) if series.year else ft.Container(),
            ft.Text(series.rating, color=ft.Colors.ON_SURFACE_VARIANT, size=13) if series.rating else ft.Container(),
        ],
        spacing=16,
    )

    series_info = ft.Container(
    ...
    scroll_content = ft.Column(
        controls=[header, series_info, episodes_list],
        expand=False,
        spacing=0,
    )
    ...
    view = ft.View(
        route=f"/series/{series.nkiri_id}",
        controls=[
            ft.SafeArea(
                ft.Container(
                    content=scrollable,
